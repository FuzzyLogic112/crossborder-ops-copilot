...
完整运营链路。**面板的 Tab 顺序就是这个流程顺序**：

```
① 抓取数据 → ② 竞品数据 → ③ 选品工作台 → ④ 供应商比价
→ ⑤ 选品评分 → ⑥ 定价计算器 → ⑦ 文案生成 → ⑧ 变化监控
额外：配置 | 数据存储 | 说明
```

## 数据可靠性分级

**用之前必须知道哪些数是真的、哪些是占位值。** 这个分级在代码注释和面板界面上都标了。

| 数据 | 可靠性 | 说明 |
|---|---|---|
| **竞品数据**（价格/评分/评价数/排名） | ✅ **真实抓取** | 来自 Amazon 公开 Best Sellers 榜单页，自有工具 amzrank（Playwright，3 秒间隔） |
| **平台佣金率 / 支付费率** | ✅ **真实公开数据** | 查的各平台公开费率表，**来源链接写在 `data/platforms.yaml` 文件头注释里** |
| **类目 slug 可用性** | ✅ **逐个实测** | 打开榜单页数商品卡片验证。美国站 20 个可用 / 7 个确认不可用 |
| **退货率** `return_rate` | ⚠️ **估算占位值** | 平台不公布，只能用自己店铺历史订单统计后替换 |
| **物流费率** | ⚠️ **估算占位值** | 货代一对一议价、不公开挂牌，需向货代索取 rate card 后替换 |
| **汇率** | ⚠️ **占位量级** | 不是实时汇率，用前须自行核实当日汇率 |
| **供应商报价** | ⚠️ **人工录入** | 工具不抓供货平台，原因见「为什么不自动抓取供货平台」 |
| `data/` 根目录的 `products.csv` 与 `competitors_*.csv` | ⚠️ **训练用虚构数据** | URL 为不可路由占位域名。保留是为了让测试与演示不依赖网络 |

**任何情况下都不代表真实店铺业绩、真实 GMV 或真实投放结果。**

### 两个已知的模型局限

**① 含固定费的平台会被低估成本。** eBay 每单 $0.30、Etsy 每单 $0.25 加每 listing $0.20 ——
本模型**只算百分比费率，未计入固定费**。低客单价商品尤其要注意。

**② 采购成本、重量、退货率抓不到。** 那是供应链与店铺自有数据，
只能靠供应商报价和历史订单填入。

### 一个值得注意的费率发现

**Shopee 跨境比本地贵一倍多**：本地 2–8%，但新加坡跨境约 14%、马来西亚 18.36%，
且 2026-02 起新马泰越**再加 5% 技术服务费**。
已按跨境费率单独建配置项（`shopee_crossborder_sg` 19%、`shopee_crossborder_my` 23.4%）。

## 全部可自定义，不需要改代码

平台、市场、站点三类清单都在 `data/*.yaml`，面板「配置」页可直接编辑，
**保存即刻生效，无需重启**。写坏了会自动回滚。

| 配置 | 文件 | 已内置 |
|---|---|---|
| 销售平台费率 | `data/platforms.yaml` | **16 个**：Amazon（标准/电子/服饰）、TikTok Shop（常规/新卖家）、独立站（Stripe/PayPal）、eBay、Walmart（常规/低费率）、Etsy、Temu、Shopee（本地/新加坡跨境/马来跨境）、Lazada |
| 目的市场物流 | `data/logistics_rates.yaml` | **10 个**：US / EU / SEA / UK / JP / CA / AU / MX / BR / ME |
| 抓取站点 | `data/sites.yaml` | **4 个可抓**（us / jp / uk / de）+ 5 个已列出待启用 |
| 类目 slug | 不需配置 | **本来就自由填**，只校验格式不限取值 |
